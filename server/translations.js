const resources = {
  en: {
    translation: {
      "Not authorized" : "Not authorized",
      "Not logged in" : "Not logged in",
      "existing" : "existing",
      "Invalid URL" : "Invalid URL",
      'Closed community, can only give to' : 'Closed community, can only give to',
      "emails and/or via Slack" : "emails and/or via Slack",
      
      // Slack
      "help" : "help",
      "ykarma_is" : "YKarma is a reputation cryptocurrency which you give to others, who can then use it to buy rewards. You currently have",
      "karma to give away to others, and" : "karma to give away to others, and",
      "to_spend_on" : 'to spend on rewards. To send karma, use this slash command; for example, to send 10 karma to Alice with the message _for being awesome_, just type "/yk 10 to @alice for being awesome"',
      "Sent!" : "Sent!",
      "Sending…" : "Sending…",
      "Sorry! Valid amount not found" : "Sorry! Valid amount not found",
      "Sorry! Valid recipient not found" : "Sorry! Valid recipient not found",
      "Sorry! Your YKarma account is not set up for sending here" : "Sorry! Your YKarma account is not set up for sending here",
      "Sorry! You don't have enough YKarma to do that. Your balance is" : "Sorry! You don't have enough YKarma to do that. Your balance is",
      "Sorry! That YKarma account is not set up for receiving here" : "Sorry! That YKarma account is not set up for receiving here",
      "balance" : "balance",
      "bot_help" : "You can check your balance with 'balance', send with 'send' eg 'send 10 to @alice for being awesome', view available rewards with 'rewards', or purchase a reward with 'purchase' eg 'purchase 23'",
      "Error getting available-to-spend amount" : "Error getting available-to-spend amount",
      "You currently have a total of" : "You currently have a total of",
      "to spend… You can get an itemization by flavor with the 'flavors' command" : "to spend… You can get an itemization by flavor with the 'flavors' command",
      "You currently have" : "You currently have",
      "to give away" : "to give away",
      "send" : "send",
      "has sent you" : "has sent you",
      "karma" : "karma",
      "with the message" : "with the message",
      "rewards" : "rewards",
      'Fetching available rewards from the blockchain…' : 'Fetching available rewards from the blockchain…',
      "Error getting rewards" : "Error getting rewards",
      "No rewards available" : "No rewards available",
      "Available Rewards" : "Available Rewards",
      "Error getting list of rewards" : "Error getting list of rewards",
      "purchase" : "purchase",
      "Please tell me the ID of the reward you want to purchase" : "Please tell me the ID of the reward you want to purchase",
      "Attempting purchase…" : "Attempting purchase…",
      "flavors" : "flavors",
      "Sorry, I didn't understand you. You can ask for help with 'help'" : "Sorry, I didn't understand you. You can ask for help with 'help'",
      "leaderboard" : "leaderboard",
      "You can also view the current leaderboard with 'leaderboard'" : "You can also view the current leaderboard with 'leaderboard'",
      "Could not get leaderboard, sorry!" : "Could not get leaderboard, sorry!",
      "Getting leaderboard…" : "Getting leaderboard…",
      "Available rewards" : "Available rewards",
      "To purchase, enter 'purchase' followed by the reward's ID number, e.g. 'purchase 7'" : "To purchase, enter 'purchase' followed by the reward's ID number, e.g. 'purchase 7'",
      
      // Emails
      
      
    }
  },
  kr: {
    translation: {
      
       "Not authorized" : "승인되지 않음",
      	"Not logged in" : "로그인되지 않음",
      	"existing" : "현재 사용되는",
      	"Invalid URL" : "요효하지 않은 URL",
      	'Closed community, can only give to' : '비공개 커뮤니티만 보낼 수 있습니다.',
      	"emails and/or via Slack" : "이메일 이나 슬랙을 통해서",
      
      // Slack
      "help" : "help",
      "ykarma_is" : "Karma는 다른사람에게 줄 수 있는  평판암호화폐로,  리워드를 얻기 위해 Karma을 사용할 수 있습니다.  현재 보유하고 있는 카르마",
      "karma to give away to others, and" : "Karma는 다른 사람에게 나눠 줄 수 있고,",
      "to_spend_on" : '리워드에 사용할 수 있습니다. karma를 보개기 위해서는 , 이 슬래시 명령어를 사용하면 됩니다. 예를 들어, 10 Karma를 엘리스에게 being awesome 메세지와 함께 보낸다면, 이렇게 입력하면 됩니다. "/yk 10 to @alice for being awesome"',
      "Sent!" : "보냄",
      "Sending…" : "보내는중…",
      "Sorry! Valid amount not found" : "죄송합니다! 유효한 금액을 찾지 못했습니다.",
      "Sorry! Valid recipient not found" : "죄송합니다.! 유효한 수신자를 찾지 못했습니다.",
      "Sorry! Your YKarma account is not set up for sending here" : "죄송합니다.  이곳에서 보낼 수 있도록 YKarma 계정이 설정되어 있지 않습니다.",
      "Sorry! You don't have enough YKarma to do that. Your balance is" : "죄송합니디. Karma가 충분하지 않습니다. 현재 보유 Karma는",
      "Sorry! That YKarma account is not set up for receiving here" : "죄송합니다.  이곳에서 받을 수 있도록 YKarma 계정이 설정되어 있지 않습니다.",
      "balance" : "잔고",
      "bot_help" : "잔고를 'balance' 명령어로 확인할 수 있습니다. 보낼 때는  'send' 입니다. 예를 들어  'send 10 to @alice for being awesome',  리워드를 보려면  'rewards' 명령어를 입력하시면 됩니다. 리워드를 사고자 한다면  'purchase' 입니다. 예를 들면 'purchase 23'",
      "Error getting available-to-spend amount" : "지출 가능 금액을 가져 오는 중 오류가 발생했습니다.",
      "You currently have a total of" : "현재 보유하고 있는 총액은",
      "to spend… You can get an itemization by flavor with the 'flavors' command" : "사용을 위해서… 선호하는 항목을  'flavors' 명령어를 통해서 얻을 수 있습니다.",
      "You currently have" : "현재 보유하고 있는",
      "to give away" : "보내기 위해서",
      "send" : "보내기",
      "has sent you" : "가 보냈습니다.",
      "karma" : "karma",
      "with the message" : "메세지와 함께",
      "rewards" : "리워드",
      'Fetching available rewards from the blockchain…' : '블록체인에서 사용 가능한 리워드를 가져오는 중입니다…',
      "Error getting rewards" : "리워드를 불러오는데 실패했습니다",
      "No rewards available" : "이용가능한 리워드가 없습니다.",
      "Available Rewards" : "이용가능한 리워드",
      "Error getting list of rewards" : "리워드 목록을 볼러오는데 오류가 발생했습니다.",
      "purchase" : "구매하기",
      "Please tell me the ID of the reward you want to purchase" : "구매를 원하는 리워드의 ID를 알려주세요.",
      "Attempting purchase…" : "구매를 진행하고 있습니다…",
      "flavors" : "flavors",
      "Sorry, I didn't understand you. You can ask for help with 'help'" : "죄송합니다. 도움말은 'help' 명령어를 사용하시면 됩니다.",
      
      // Emails
        
    }
  }
};

module.exports = resources;
